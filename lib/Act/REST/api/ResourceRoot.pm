package Act::REST::api::ResourceRoot;

=head1 NAME

Act::REST::api::ResourceRoot
...
sub new_from_http_request {
    ...
    if ($evnt) {
        Role::Tiny->apply_roles_to_object($resource_root,
            ('Act::REST::api::ResourceRoot::Role::Event'));
    }
    elsif ($edtn) { # that is odd, how can a edition NOT be an event?
        Role::Tiny->apply_roles_to_object($resource_root,
            ('Act::REST::api::ResourceRoot::Role::Edition'));
    }
    elsif ($synd) {
        Role::Tiny->apply_roles_to_object($resource_root,
            ('Act::REST::api::ResourceRoot::Role::Syndicate'));
    }
    else {
        Role::Tiny->apply_roles_to_object($resource_root,
